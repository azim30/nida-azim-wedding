...
document.getElementById("openInvite").addEventListener("click",()=>{
  ...
  // Start music immediately from the user's tap/click.
  // This is the reliable approach on iPhone/Safari because the
  // Open Invitation button itself is a user-initiated gesture.
  audio.play()
    .then(()=>document.getElementById("musicBtn").textContent="❚❚")
    .catch(()=>{
      // If the browser still blocks playback, the top-right music
      // button remains available as the fallback.
